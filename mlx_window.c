#include "mlx.h"
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
#include "fractol.h"

int key_hook(int keycode, t_data *img)
{
    printf("%d\n", keycode);
    if (keycode == 65307) // macOS ESC
    {
        mlx_destroy_window(img->mlx, img->win);
        exit(0);
    }
    else if (keycode == 65362) // ↑
        img->offset_y -= 0.1 / img->zoom;
    else if (keycode == 65364) // ↓
        img->offset_y += 0.1 / img->zoom;
    else if (keycode == 65361) // ←
        img->offset_x -= 0.1 / img->zoom;
    else if (keycode == 65363) // →
        img->offset_x += 0.1 / img->zoom;
    else if (keycode == 65451) // +
        img->zoom *= 1.2;
    else if (keycode == 65453) // -
        img->zoom /= 1.2;
    render_mandelbrot(img);
    mlx_put_image_to_window(img->mlx, img->win, img->img, 0, 0);
    return (0);
}

int mouse_hook(int button, int x, int y, t_data *img)
{
    if (button == 4 || button == 5) // 滚轮上下
    {
        double zoom_factor = (button == 4) ? 1.2 : 1 / 1.2;

        // 获取鼠标指向的复平面位置
        double mouse_re = (x - WIDTH / 2.0) / (0.5 * img->zoom * WIDTH) + img->offset_x;
        double mouse_im = (y - HEIGHT / 2.0) / (0.5 * img->zoom * HEIGHT) + img->offset_y;

        // 更新 zoom
        img->zoom *= zoom_factor;

        // 缩放时让焦点保持在鼠标位置
        img->offset_x = mouse_re - (x - WIDTH / 2.0) / (0.5 * img->zoom * WIDTH);
        img->offset_y = mouse_im - (y - HEIGHT / 2.0) / (0.5 * img->zoom * HEIGHT);

        // 重绘
        render_mandelbrot(img);
        mlx_put_image_to_window(img->mlx, img->win, img->img, 0, 0);
    }
    return (0);
}

int mouse_click(int button, int x, int y, t_data *img)
{
    if (button == 1) // 鼠标左键按下
    {
        img->is_dragging = 1;
        img->last_x = x;
        img->last_y = y;
    }
    else if (button == 3) // 鼠标右键松开，或你可以检测释放事件
    {
        img->is_dragging = 0;
    }
    return (0);
}

int mouse_press(int button, int x, int y, t_data *img)
{
    if (button == 1) // 左键按下
    {
        img->is_dragging = 1;
        img->last_x = x;
        img->last_y = y;
    }
    else if (button == 4 || button == 5) // 滚轮事件
    {
        double mouse_re = (double)x / WIDTH * 3.0 - 2.0;
        double mouse_im = (double)y / HEIGHT * 3.0 - 1.5;

        // 将屏幕坐标转换为当前视图中的复平面坐标
        mouse_re = mouse_re / img->zoom + img->offset_x;
        mouse_im = mouse_im / img->zoom + img->offset_y;

        if (button == 4) // 滚轮向上：放大
            img->zoom *= ZOOM_FACTOR;
        else if (button == 5) // 滚轮向下：缩小
            img->zoom /= ZOOM_FACTOR;

        // 以鼠标指针为缩放中心进行坐标偏移校正
        img->offset_x = mouse_re - ((double)x / WIDTH * 3.0 - 2.0) / img->zoom;
        img->offset_y = mouse_im - ((double)y / HEIGHT * 3.0 - 1.5) / img->zoom;

        img->need_redraw = 1;
    }
    return (0);
}

int mouse_release(int button, int x, int y, t_data *img)
{
    (void)(x + y);
    if (button == 1) // 左键松开
    {
        img->is_dragging = 0;
    }
    return (0);
}

int mouse_move(int x, int y, t_data *img)
{
    if (!img->is_dragging)
        return (0);

    int dx = x - img->last_x;
    int dy = y - img->last_y;

    img->offset_x -= (double)dx / (0.5 * img->zoom * WIDTH);
    img->offset_y -= (double)dy / (0.5 * img->zoom * HEIGHT);

    img->last_x = x;
    img->last_y = y;

    img->need_redraw = 1;
    return (0);
}

int render_frame(t_data *img)
{
    if (!img->need_redraw)
        return (0); // 不重绘

    render_mandelbrot(img);
    mlx_put_image_to_window(img->mlx, img->win, img->img, 0, 0);
    img->need_redraw = 0; // ✅ 重置
    return (0);
}

int close_window(t_data *img)
{
    mlx_destroy_window(img->mlx, img->win);
    exit(0);
    return (0);
}

int main(void)
{
    t_data img;

    img.zoom = 1.0;
    img.offset_x = -0.5;
    img.offset_y = 0.0;
    img.max_iter = MAX_ITER;
    img.mlx = mlx_init();
    img.win = mlx_new_window(img.mlx, WIDTH, HEIGHT, "Mandelbrot");
    img.img = mlx_new_image(img.mlx, WIDTH, HEIGHT);
    img.addr = mlx_get_data_addr(img.img, &img.bpp, &img.line_len, &img.endian);
    mlx_hook(img.win, 2, 1L << 0, key_hook, &img); // 键盘按下
    mlx_hook(img.win, 17, 0, close_window, &img);  // 关闭窗口

    mlx_hook(img.win, 4, 1L << 2, mouse_press, &img);   // 鼠标按下
    mlx_hook(img.win, 5, 1L << 3, mouse_release, &img); // 鼠标松开
    mlx_hook(img.win, 6, 1L << 6, mouse_move, &img);    // 鼠标移动
    mlx_loop_hook(img.mlx, render_frame, &img);
    mlx_hook(img.win, 6, 1L << 6, mouse_move, &img); // 鼠标移动

    mlx_loop_hook(img.mlx, render_frame, &img);
    render_mandelbrot(&img);
    mlx_put_image_to_window(img.mlx, img.win, img.img, 0, 0);
    mlx_loop(img.mlx);
    return (0);
}
